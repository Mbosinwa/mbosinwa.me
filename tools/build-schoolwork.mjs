#!/usr/bin/env node
// Generator for the /schoolwork/ section of mbosinwa.dev.
//
...
//   node tools/build-schoolwork.mjs
//
...
const OUT = new URL('../schoolwork/', import.meta.url).pathname;
...
function head({ title, desc, canonical }) {
  return `<meta charset="UTF-8">
    ...
    <link rel="stylesheet" href="/schoolwork/schoolwork.css">`;
}
...
const NAVBAR = `<nav class="navbar">
        <div class="nav-content">
            ...
            <div class="nav-right">
                ...
                <a href="/schoolwork/" class="nav-link">Schoolwork</a>
...
function docPage(prog, sem, course, pg, bodyHtml) {
  const base = `/schoolwork/${prog.slug}/${sem.slug}/${course.slug}`;
  return page({
    title: `${course.code} ${pg.title} — Schoolwork — Mbosinwa Awunor`,
    desc: `${course.code} ${course.title}: ${pg.desc}`,
    canonical: `${SITE}${base}/${pg.out}`,
    body: `        ${crumb([['Schoolwork', '/schoolwork/'], [prog.name, `/schoolwork/${prog.slug}/`], [sem.name, `/schoolwork/${prog.slug}/${sem.slug}/`], [`${course.code} ${course.title}`, `${base}/`], [pg.title]])}
        <article class="doc">
${bodyHtml}
        </article>`,
  });
...
${course.volumes.map((v) => `                <div class="material-item">
  ...
  return page({
    title: `${course.code} — ${course.title} — Schoolwork — Mbosinwa Awunor`,
    desc: `${course.title} study materials: ${course.blurb}`,
    canonical: `${SITE}${base}/`,
    body: `        ${crumb([['Schoolwork', '/schoolwork/'], [prog.name, `/schoolwork/${prog.slug}/`], [sem.name, `/schoolwork/${prog.slug}/${sem.slug}/`], [`${course.code} ${course.title}`]])}
...
function semesterPage(prog, sem) {
  const cards = sem.courses.map((c) => {
    ...
    return `            <a class="course-card" href="/schoolwork/${prog.slug}/${sem.slug}/${c.slug}/">
  ...
  return page({
    title: `${sem.name} — ${prog.name} — Schoolwork — Mbosinwa Awunor`,
    ...
    canonical: `${SITE}/schoolwork/${prog.slug}/${sem.slug}/`,
    body: `        ${crumb([['Schoolwork', '/schoolwork/'], [prog.name, `/schoolwork/${prog.slug}/`], [sem.name]])}
        <header class="page-header">
...
function programmePage(prog) {
  const cards = prog.semesters.map((s) => {
    ...
    return `            <a class="course-card" href="/schoolwork/${prog.slug}/${s.slug}/">
  ...
  return page({
    title: `${prog.name} — Schoolwork — Mbosinwa Awunor`,
    desc: `Study materials from my ${prog.name} at ${prog.school}, organised by semester.`,
    canonical: `${SITE}/schoolwork/${prog.slug}/`,
    body: `        ${crumb([['Schoolwork', '/schoolwork/'], [prog.name]])}
        <header class="page-header">
...
function landingPage() {
  const cards = PROGRAMMES.map((p) => {
    ...
    return `            <a class="course-card" href="/schoolwork/${p.slug}/">
  ...
  return page({
    title: 'Schoolwork — Study Materials — Mbosinwa Awunor',
    ...
    canonical: `${SITE}/schoolwork/`,
    body: `        <header class="page-header">
            <span class="kicker">Study Materials</span>
            <h1>Schoolwork</h1>
            <p class="lede">Materials I produce while studying Computer Science at postgraduate level — transcribed lecture notes, cheatsheets, practice questions and multi-volume exam study guides, organised by programme and semester. All self-authored, all free to use.</p>
        </header>
        <div class="card-grid">
${cards}
        </div>`,
